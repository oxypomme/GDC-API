...
const fetchAllMissions = async () => {
  ...
  fs.writeFileSync(
    "db/data/missions.json",
    JSON.stringify({ missions, updated: new Date() })
  );
};
...
const getAllMissions = async () => {
  ...
  if (dayjs().isAfter(dayjs(missionsJSON.updated).add(1, "h"))) {
    await fetchAllMissions();
  }
  return missionsJSON;
};
...
const getMission = async (id) => {
  ...
  try {
    ...
    if (!mission || dayjs().isAfter(dayjs(mission.updated).add(1, "h"))) {
